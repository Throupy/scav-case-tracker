...
async def case_types(ctx):
    ...
    for valid_type, variations in valid_types.items():
        variations_str = ', '.join(variations)  # Convert list of variations to a comma-separated string
        embed.add_field(name=f"__{valid_type}__ - {variations_str}", value="", inline=False)
...
class ImageDownloaderClient(commands.Bot):
                ...
                for attachment in message.attachments:
                    if attachment.url.split("?")[0].endswith(('jpg', 'jpeg', 'png')):
                        ...
                        await self.download_image(
                            message, attachment, matched_type, status_embed, status_message
                        )

        await self.process_commands(message)

    async def download_image(self, message, attachment, scav_case_type, status_embed, status_message):
        try:
            # Update the message with current progress
            status_embed.description = "Image downloaded. Performing OCR and retrieving prices..."
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(attachment.url) as response:
                    if response.status == 200:
                        ...
                        await self.submit_image_to_flask(message, file_path, scav_case_type, status_embed, status_message)
                    else:
                        status_embed.description = f"Failed to download image: {attachment.filename}"
                        await status_message.edit(embed=status_embed)
    ...
    async def submit_image_to_flask(self, message, image_path, scav_case_type, status_embed, status_message):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                with open(image_path, 'rb') as image_file:
                    ...
                    form_data.add_field('image', image_file, filename=os.path.basename(image_path))
                    ...
                    async with session.post(url, headers=headers, data=form_data) as response:
                        if response.status == 200:
                            ...
                            status_embed.description = f"Failed to process image: {error_msg['error']}"
                            await status_message.edit(embed=status_embed)
        except Exception as e:
